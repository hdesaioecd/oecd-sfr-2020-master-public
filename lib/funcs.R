cluster = function(pca, num.clusters = 6, cluster.method = "ward.D2", round.numbers = T){
  ...
  tmp <- dplyr::left_join(tmp, select(centroids, cut, cluster),by = "cut")
...
interpolate = function(df){
  ...
  if(diff(range(df$year))>0){
    num.years.test = df %>% 
      ...
      dplyr::summarise(only.one.yr = min(year) == max(year), year = min(year), value = max(value)) %>%
      filter(only.one.yr)
...
add.zeros.for.missing.countries = function(x, raw.data, replace.with = 0)
{
  ...
  temp = lapply(raw.data[-1], function(x) return(data.frame(iso3c = unique(as.character(x$iso3c)))))
  temp = bind_rows(temp)
...
impute = function(raw.data, use.precomputed = F)
{
  ...
  if(!use.precomputed){
    ...
    #combine ids with x
    x = left_join(x, ids)
    x = x %>% dplyr::select(-variablename) %>% spread(uid, value) %>% as.data.frame() 
    ...
    precomputedid = paste0(pre.computed.imputations$iso3c, 
...
country.code.name = function(x, oecd = F)
{
  ...
  {
    y = countrycode(x, "iso3c", "country.name",custom_match = c("KSV" = "Kosovo",
                                                                "XKX" = "Kosovo",
                                                                "UVK" = "Kosovo",
                                                                "CHI" = "Channel Islands",
                                                                "SWZ" = "Eswatini",
  ...
  }else
  {
    ...
    {
      y = countrycode(x, "country.name", "iso3c",custom_match = c("Kosovo" = "XKX",
