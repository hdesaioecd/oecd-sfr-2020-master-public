...
for (the.pca in names(all.pca)[1:5]){ #1:5 refers to all the dimensions
    ...
    temp = left_join(sfr.time.series %>% dplyr::filter(dimension == the.pca), 
                 tmp %>% dplyr::select(iso3c, cluster),by = "iso3c")
    temp = temp %>% dplyr::filter(year == max(year)) #for the latest year found in time series,
