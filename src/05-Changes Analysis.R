...
write.xlsx(sfr.time.series_export,paste0(output_folders,"export of indicators (with imputed values).xlsx"))
...
population_filtered <- read_excel("./data/population data/world_pop_2018_timeseries.xlsx",
  ...
  dplyr::mutate(value = as.numeric(value)*1000,
                year = gsub("x","",year),
...
ODA_recipients <- read_xls("./data/additional data/DAC-CRS-CODES.xls",sheet = "Recipient") %>% 
  ...
  dplyr::mutate(iso3c = countrycode(recipient_name_en,"country.name.en","iso3c",
                                    custom_match = c("Eswatini" = "SWZ",
                                                     "Kosovo" = "XKX",
                                                     "Micronesia" = "FSM",
...
oecd_classifications <- read_xls("./data/additional data/DAC-CRS-CODES.xls",
                                 sheet = "Recipient") %>%
  clean_names() %>% 
  filter(dac_income_group != "Part I unallocated by income") %>% 
  dplyr::mutate(iso3c = countrycode(recipient_name_en,"country.name","iso3c",
                                    custom_match = c("Eswatini" = "SWZ",
                                                     "Kosovo" = "XKX",
                                                     "Micronesia" = "FSM",
...
oecd_class_income <- oecd_classifications %>% dplyr::select(iso3c,group = dac_income_group)

#region classifications, with income classifications r-bind
oecd_class <- oecd_classifications %>% dplyr::select(iso3c,group = region) %>% 
...
oecd_fragile <- aggr.pca_adj %>% 
  ...
  dplyr::mutate(group = gsub("Fragile","Other Fragile",group),
...
wb_classifications <- read_xls("./data/additional data/WB_2020_classifications.xls",
                               sheet = "Groups") %>%
  dplyr::mutate(iso3c = countrycode(CountryName,"country.name","iso3c",
                                    custom_match = c("Eswatini" = "SWZ",
                                                     "Kosovo" = "XKX",
                                                     "Romania" = "ROM"))) %>% 
...
subregions <- read_excel("./data/additional data/List of sub-regions.xlsx") %>% 
...
commodities_list <- read_excel("./data/additional data/cddc.xlsx") %>% 
  mutate(iso3c = countrycode(country,"country.name","iso3c",custom_match = c("Micronesia FSM" = "FSM")))

commodities_all <- commodities_list %>% mutate(group = "Commodity dependence") %>% select(-country)

commodities_disaggregated <- commodities_list %>% select(iso3c,group)

#import custom list of conflict countries from UCDP
conflict_list <- fread("./data/additional data/UCDP Conflict Intensity.csv")

conflict_list_all <- conflict_list %>% dplyr::mutate(group = "In conflict") %>% 
  rbind(conflict_list)
...
climate_change <- fread("./data/additional data/exposure to climate change.csv")

climate_change_frag <- climate_change %>% 
  filter(iso3c %in% unique(aggr.pca_adj$iso3c))

conflict_and_climate <- conflict_list_all %>% filter(group == "In conflict") %>% 
...
eiu_raw <- fread("./data/additional data/governance_index.csv") %>%
  dplyr::mutate(iso3c = countrycode(country,"country.name","iso3c",custom_match = c("eSwatini" = "SWZ"))) %>% 
  select(iso3c,group = type)

#rbind entire list
iso_bygroup <- rbind(oecd_class,wb_class,oecd_fragile,oecd_fragile_all,oecd_nonfragile,subregions %>% dplyr::select(-c(countries,source)),
                     ODA_recipients %>% dplyr::select(iso3c,group = category),
                     commodities_all,commodities_disaggregated,conflict_list_all,climate_change,
                     conflict_and_climate,eiu_raw)
...
iso_counts_wpop <- iso_bygroup %>% 
  dplyr::left_join(population_filtered %>% 
                     dplyr::filter(year == 2020) %>% 
                     dplyr::select(iso3c,population),by = "iso3c") %>% 
  dplyr::filter(complete.cases(.))
...
iso_counts_wpop_fragile_bygroup <- iso_counts_wpop_fragile %>% 
  ...
  dplyr::mutate(total_frag_pop = pop_fragile$total_frag_pop,
                proportion_of_frag = population_fragile/total_frag_pop*100)

iso_counts_export <- list("Countries" = iso_counts_fragile_bygroup %>% dplyr::select(Groups = group,
                                                                                     `Fragile Contexts` = count_fragile,
                                                                                     `Total Contexts` = count_total,
                                                                                     Proportion = proportion),
                          "Population" = iso_counts_wpop_fragile_bygroup %>% 
                            dplyr::select(Groups = group,
...
write.xlsx(iso_counts_export,paste0(output_folders,"Dataset - Country Representation by Group.xlsx"))
...
clusters <- clusters %>% dplyr::mutate(country = countrycode(iso3c,"iso3c","un.name.en",custom_match = c("PSE" = "Palestine",
                                                                                                         "ROM" = "Romania",
                                                                                                         "SWZ" = "Eswatini")))
colours <- mean.plot %>% dplyr::rename(color = colour) %>% dplyr::select(cluster,color) %>% distinct()
clusters_adj <- clusters %>% dplyr::left_join(colours,by = "cluster") %>% dplyr::select(dimension,iso3c,country,color) %>% 
...
write.xlsx(clusters_split,paste0(output_folders,"clusters/clusters.xlsx"))
...
#compute proportion of each cluser that has fragile contexts  
clusters_aggregated <- clusters_export %>% 
  left_join(iso_bygroup,"iso3c") %>% 
  group_by(dimension,description,group) %>% 
  summarise(n = n()) %>% 
  left_join(clusters_total_count %>% rename(total = n),c("dimension","description")) %>% 
  mutate(proportion = n/total*100) %>% 
  split(.$dimension)

write.xlsx(clusters_aggregated,paste0(output_folders,"clusters/Cluster Aggregations.xlsx"))
write.xlsx(clusters_export,paste0(output_folders,"clusters/Clusters with Colors.xlsx"))

clusters_export <- clusters_export %>% 
  mutate(shading = ifelse(grepl("Extreme",description),"Darkest Shade",
                          ifelse(grepl("High",description),"Second-darkest shade",
                                 ifelse(grepl("Moderate",description),"Third-darkest shade",
                                        ifelse(grepl("Low",description),"Fourth-darkest shade",
                                               "Lightest shade")))))
clusters_export_for_designer <- aggr.pca_adj %>%
  select(country = location,iso3c) %>% 
  left_join(clusters_export %>% select(iso3c,dimension,shading),"iso3c") %>% 
  pivot_wider(names_from = "dimension",values_from = "shading")
write.xlsx(clusters_export_for_designer,paste0(output_folders,"clusters/Clusters for Designer.xlsx"))

# Rankings of each country by dimension - additional data check -----------------------------------
raw.data_rankings <- raw.data %>% 
  dplyr::select(iso3c,year,ends_with("PC1")) %>% 
  dplyr::mutate(location = countrycode(.$iso3c,"iso3c","un.name.en",
                                       custom_match = c("PSE" = "West Bank and Gaza Strip",
                                                        "SWZ" = "Eswatini",
                                                        "ROM" = "Romania"))) %>% 
  dplyr::select(iso3c,location,everything()) %>% 
  dplyr::group_by(year) %>% 
  dplyr::mutate(econ_rank = rank(Economic.PC1,ties.method = 'first'),
  ...
  dplyr::left_join(aggr.pca_adj[,c("iso3c","fragility.level")],by = "iso3c") %>%
  dplyr::mutate(fragility.level = replace_na(fragility.level,"Rest of the World")) %>%
  dplyr::left_join(ODA_recipients[,c("iso3c","category")],by = "iso3c") %>%
  dplyr::mutate(category = replace_na(category,"Non-ODA eligible")) %>% 
  dplyr::select(iso3c,location,year,fragility.level,ODA_eligibility = category,ends_with("rank")) %>% 
  split(.$year)
...
write.xlsx(raw.data_rankings,paste0(output_folders,"Rankings - Context, by Dimension and Year.xlsx"))
...
list_2018 <- list_2018_raw %>% dplyr::filter(year == 2017) %>% dplyr::select(iso3c,Aggregate.PC1) %>% 
  dplyr::filter(Aggregate.PC1 < -1.20) %>%
  dplyr::mutate(country = countrycode(iso3c,"iso3c","country.name",custom_match = c("SWZ" = "Eswatini")))
list_2016 <- fread("./data/additional data/list_2016.csv") %>% dplyr::filter(fragility.level %in% c("Fragile","Extreme Fragility")) %>% 
  dplyr::mutate(country = countrycode(iso3c,"iso3c","country.name",custom_match = c("SWZ" = "Eswatini")))
list_2020 <- aggr.pca_adj %>% dplyr::filter(Fragility < -1.20) %>% 
  dplyr::mutate(country = countrycode(iso3c,"iso3c","country.name",custom_match = c("SWZ" = "Eswatini")))
...
incl_2018 <- as.data.frame(matrix(setdiff(list_2018$country,list_2020$country),dimnames = list(NULL,c("Countries Moving off the List"))))

#countries that were not on the 2018 list but are on the 2020 list
excl_2018 <- as.data.frame(matrix(setdiff(list_2020$country,list_2018$country),dimnames = list(NULL,c("Countries Moving onto the List"))))

changes_from_2018 <- cbindPad(incl_2018,excl_2018)

#countries that were on the 2016 list but are not on the 2020 list
incl_2016 <- as.data.frame(matrix(setdiff(list_2016$country,list_2020$country),dimnames = list(NULL,c("Countries on 2016 list, Not on 2020 list"))))

#countries that were not on the 2016 list but are on the 2020 list
excl_2016 <- as.data.frame(matrix(setdiff(list_2020$country,list_2016$country),dimnames = list(NULL,c("Countries on 2020 list, Not on 2016 list"))))
...
write.xlsx(changes_bt_lists,paste0(output_folders,"changes/Changes Between Lists.xlsx"))
...
lapply(names(contrib.bar.viz),
       function(x) ggsave(filename = paste0("./graphs/contributions/PC1and2/",x,".jpg",sep = ""),
                          plot = contrib.bar.viz[[x]],width = 10, height = 10))
...
lapply(names(contrib.bar.viz.Dim1),
       function(x)ggsave(filename = paste0("./graphs/contributions/PC1/",x,".jpg",sep = ""),
                         plot = contrib.bar.viz.Dim1[[x]],width = 12, height = 12))
...
lapply(names(contrib.bar.viz.Dim2),
       function(x)ggsave(filename = paste0("./graphs/contributions/PC2/",x,".jpg",sep = ""),
                         plot = contrib.bar.viz.Dim2[[x]],width = 12, height = 12))
...
matching_dataset <- aggr.pca.full %>% 
  dplyr::select(iso3c,description) 
string_split <- str_split_fixed(matching_dataset$description, " ",3) %>% 
  as.data.frame()
matching_dataset <- cbind(matching_dataset,string_split) %>% dplyr::rename(severity = V1,
                                                                           dimension = V2) %>% 
  dplyr::select(-c(description,V3)) %>% spread(dimension,severity) %>% 
  dplyr::mutate(iso3c = as.character(iso3c))

export_dimensional_analysis <- aggr.pca.full %>% 
  dplyr::select(iso3c,description,fragility.level) %>% 
  dplyr::filter(complete.cases(.),!grepl("Aggregate",description)) %>% 
  dplyr::select(iso3c,description)
fwrite(export_dimensional_analysis,paste0(output_folders,"Dimensional Categories for ODA Analysis.csv"))
...
fwrite(matches_ordered,paste0(output_folders,"Matches Across Clusters - by Context.csv"))
...
agg_overtime_pop_weighted <- agg %>% 
  left_join(population_filtered %>% mutate(year = as.character(year)),by = c("year","iso3c")) %>% 
  mutate(category = ifelse(value < -2.5,"Extremely Fragile",
                           ifelse(value < -1.2,"Moderately Fragile",
                                  "Non-fragile"))) %>% 
...
export_agg_overtime <- list("All" = agg_overtime_pop_weighted_all,
                            "Disaggreated" = agg_overtime_pop_weighted_disag)
write.xlsx(export_agg_overtime,paste0(output_folders,"changes/Changes in Fragility - shifting definitions.xlsx"))
...
overtime <- setDT(agg)[,.(iso3c,value,
                       ...
                       ][year >= 2012,.(year,iso3c = as.character(iso3c),fragile,extreme_fragile)] #subset greater than 2012
overtime$country <- country.code.name(overtime$iso3c)
overtime_fragile <- overtime %>% dplyr::select(-extreme_fragile) %>% dplyr::mutate(fragile = as.numeric(fragile)) %>% spread(year,fragile) %>% janitor::adorn_totals()
overtime_extremely_fragile <- overtime %>% dplyr::select(-fragile) %>% dplyr::mutate(extreme_fragile = as.numeric(extreme_fragile)) %>% 
...
library(openxlsx)
write.xlsx(list_overtime_categories,paste0(output_folders,"changes/Changes between Categories Over time.xlsx"))
...
avg_fragility <- overtime_numerical %>% 
  dplyr::filter(fragility %in% c("Extremely Fragile","Other Fragile")) %>% 
  dplyr::left_join(wb_classifications,by = "iso3c")

avg_fragility_agg <- avg_fragility %>% 
  dplyr::group_by(year,group = GroupName) %>% 
  dplyr::summarise(value = mean(value,na.rm = T))

avg_fragility_byIG <- avg_fragility_agg %>% 
  dplyr::filter(group %in% c("High income","Upper middle income",
                             "Lower middle income","Low income"))

avg_fragility_byIG_plot <- ggplot(avg_fragility_byIG %>% dplyr::filter(year >= 2013),
                                  aes(x = year,
                                      y = -value,
                                      group = group,
                                      color = group)) +
...
avg_fragility_byreg <- avg_fragility_agg %>% 
  dplyr::filter(group %in% c("East Asia & Pacific",
                             "Europe & Central Asia",
...
ggplot(avg_fragility_byreg %>% dplyr::filter(year >= 2013),
       aes(x = year,
           y = -value,
           group = group,
           color = group)) +
...
ggsave("./graphs/regional_fragility/regional_overtime_trends_forfragilecontexts.png",width = 8)

# Dimensional and Variable Changes -----------------------------------------------------------------
### Dimensional-level Deltas -------------------------------------------------------------
dimensional_changes_disag_all <- raw.data %>% dplyr::mutate(year = as.numeric(year)) %>% arrange(-year) %>%  
  gather(variablename, value, -c(iso3c, year)) %>% dplyr::filter(grepl("PC1", variablename))

deltas_all <- hpc.change(dimensional_changes_disag_all) %>% dplyr::filter(from == 2016,
                                                                          to == 2019) %>% 
...
deltas_all <- deltas_all %>% dplyr::left_join(aggr.pca_adj[,c("iso3c","fragility.level")],by = "iso3c") %>% 
  dplyr::select(iso3c,location,fragility.level,everything()) %>% 
  dplyr::mutate(fragility.level = gsub("Fragile","Other Fragile",fragility.level),
...
deltas_fragile_agg_deterioration <- deltas_all %>% dplyr::filter(variablename == "Aggregate.PC1",
...
deltas_fragile_other_deterioration <- deltas_all %>% dplyr::filter(variablename != "Aggregate.PC1",
...
write.xlsx(deltas_fragile_deterioration,paste0(output_folders,"changes/Deterioration within Each Dimension - by Context.xlsx"))
...
deltas_fragile_agg_improvement <- deltas_all %>% dplyr::filter(variablename == "Aggregate.PC1",
                                                               change == "Improvement",
                                                               fragility.level %in% c("Other Fragile",
                                                                                      "Extremely Fragile"))
##prepare a subset of all other dimensions, using the previous subsets - you will rbind these, and then split them into a list for ease of export
deltas_fragile_other_improvement <- deltas_all %>% dplyr::filter(variablename != "Aggregate.PC1",
...
write.xlsx(deltas_fragile_improvement,paste0(output_folders,"changes/Improvement within Each Dimension - by Context.xlsx"))

### Variable-level Deltas ----------------------------------------------------------------

variable_changes_disag <- sfr.time.series %>% 
  dplyr::filter(year %in% 2012:2019) %>% 
  dplyr::mutate(location = countrycode(iso3c,
                                       "iso3c","un.name.en",
                                       custom_match = c("PSE" = "West Bank and Gaza Strip",
                                                        "ROM" = "Romania",
                                                        "SWZ" = "Eswatini")),
...
deltas_byvar_final <- hpc.change(variable_changes_disag_subset) %>%
  ...
  dplyr::select(-c(prop.growth,annual.prop.growth)) %>% 
  dplyr::left_join(variable_changes_disag %>% distinct(variablename,dimension,doesmoreincreasefragility),
                   "variablename")
...
deltas_byvar_one <- deltas_byvar_bydirectionality$`1` %>% dplyr::select(-doesmoreincreasefragility) %>% 
  dplyr::distinct()
deltas_byvar_zero <- deltas_byvar_bydirectionality$`0` %>% dplyr::select(-doesmoreincreasefragility) %>% 
  dplyr::distinct()

deltas_byvar_one$change <- ifelse(deltas_byvar_one$absolute.diff < 0,"Improvement",ifelse(deltas_byvar_one$absolute.diff == 0,
                                                                                          "No Change","Deterioration"))
deltas_byvar_zero$change <- ifelse(deltas_byvar_zero$absolute.diff < 0,"Improvement",ifelse(deltas_byvar_zero$absolute.diff == 0,
                                                                                            "No Change","Deterioration"))
...
write.xlsx(deltas_list,paste0(output_folders,"changes/Changes - All Contexts, by Variable and Dimension.xlsx"))
...
group_scores_raw <- raw.data %>%
  ...
  dplyr::mutate_at(vars(-iso3c),function(x)rescale(x,c(100,0))) %>% 
  dplyr::left_join(population_filtered %>% 
                     dplyr::filter(year == 2019) %>% 
                     dplyr::select(iso3c,population),"iso3c") %>% 
...
write.xlsx(group_scores_aggregated_raw,paste0(output_folders,"group scores (single year).xlsx"))

group_scores_raw_FS <- raw.data %>%
  dplyr::select(iso3c,year,ends_with("PC1")) %>% 
  dplyr::filter(year == 2019) %>%
  dplyr::select(-year) %>% 
  dplyr::filter(iso3c %in% unique(aggr.pca_adj$iso3c)) %>% 
...
group_scores_aggregated_raw_FS <- group_scores_raw_FS %>%
  pivot_longer(names_to = "dimension",
               values_to = "value",
               -c("iso3c","group","population")) %>% 
  dplyr::group_by(dimension,group) %>% 
  dplyr::summarise(raw_value = weighted.mean(value,population,na.rm = T))
write.xlsx(group_scores_aggregated_raw_FS,paste0(output_folders,"group scores (FS only).xlsx"))
...
dat <- group_scores_raw %>% 
  ...
  filter(group %in% c("Sub-Saharan Africa",
                      "East Asia & Pacific",
                      "Europe & Central Asia",
                      "South Asia",
                      "Middle East & North Africa",
...
for(i in names(dat)){
  ggplot(dat[[i]] %>% 
           ...
           summarise(ymin = min(value),
                     ...
                     wgtmean = weighted.mean(value,population)), aes(x = reorder(group,wgtmean))) + 
    geom_boxplot(aes(ymin = ymin,
                 ...
                 fatten = NULL,
                 outlier.shape = NA) + 
    geom_point(aes(x = group,y = wgtmean),shape = 18, 
               size = 3, fill ="white",inherit.aes = FALSE) +
    theme_classic() +
    coord_flip() +
    labs(x = "",y = "Fragility score")
  ggsave(paste0("./graphs/dimensional_fragility/",i,"_Bar_Boxplot_Scaled.pdf"),width = 10)
...
group_overtime_scores_raw_unweighted <- raw.data %>%
  ...
  ungroup() %>% 
  split(.$dimension) %>% 
  map(function(x) x %>% pivot_wider(names_from = "year",values_from = "value") %>%
        rowwise() %>%
        mutate(rate_of_change = (`2019`/`2012`-1)*100))
write.xlsx(group_overtime_scores_raw_unweighted,paste0(output_folders,"Scaled group scores - overtime (no weighting).xlsx"))
...
group_overtime_scores_raw <- raw.data %>%
  ...
  mutate(value = rescale(value,c(100,0))) %>%
  ungroup() %>% 
  dplyr::left_join(population_filtered %>% mutate(year = as.character(year)) %>% 
                     dplyr::select(year,iso3c,population),c("year","iso3c")) %>% 
  dplyr::left_join(iso_bygroup,"iso3c") %>% 
  group_by(year,dimension,group) %>% 
  summarise(value = weighted.mean(value,population,na.rm = T)) %>%
  ungroup() %>% 
  ...
  map(function(x) x %>% pivot_wider(names_from = "year",values_from = "value") %>%
        rowwise() %>%
        mutate(rate_of_change = (`2019`/`2012`-1)*100))
write.xlsx(group_overtime_scores_raw,paste0(output_folders,"Scaled group scores - overtime (weighting).xlsx"))
...
group_overtime_scores_raw_FS <- raw.data %>%
  ...
  map(function(x) x %>% pivot_wider(names_from = "year",values_from = "value") %>%
        rowwise() %>%
        mutate(rate_of_change = (`2019`/`2012`-1)*100))
write.xlsx(group_overtime_scores_raw_FS,paste0(output_folders,"Scaled group scores - overtime (weighting) FS only.xlsx"))
...
dat_global_forviz <- group_overtime_scores_raw_unweighted %>%
  map(function(x) x %>%
        ...
        pivot_longer(names_to = "year",
                     values_to = "value",-c(dimension,group)) %>% 
        filter(year != "2019",
               group %in% c("Extremely Fragile","Other Fragile","Non-fragile, ODA-eligible")) %>% 
        mutate(dimension = gsub(".PC1","",dimension)))

names(dat_global_forviz) <- c("Aggregate","Economic","Environmental",
                              "Political","Security","Societal")
dat_global_viz <- imap(dat_global_forviz,
                       ~ggplot(data = .x,
                               mapping = aes(x = year,
                                             y = value,
                                             group = group)) +
                         ...
                         theme_classic() +
                         labs(colour = "Type of context",
                              x = "",
                              y = "Fragility level",
                              title = paste0(.y," fragility, over time")) +
                         ylim(0,100)
)
ggsave("./graphs/dimensional_fragility/global_graphs.pdf", marrangeGrob(grobs = dat_global_viz,
                                                                        nrow = 1,ncol = 1))
...
dat_global_foranalysis <- dat_global_forviz %>% 
  map(function(x) x %>% 
        ...
        clean_names() %>% 
        rowwise() %>% 
        mutate(diff_ext_nonfrag = extremely_fragile-non_fragile_oda_eligible,
...
dat_global_foranalysis_combined <- bind_rows(dat_global_foranalysis,.id = "dimension") %>% 
  select(dimension,year,diff_ext_nonfrag,diff_ext_otherfrag) %>% 
  clean_names() %>% 
  pivot_longer(names_to = "type",values_to = "value",-c(dimension,year)) %>% 
  mutate(year = as.numeric(year)) %>%
  mutate(type = gsub("diff_ext_nonfrag","Extremely vs. non-Fragile Contexts",type),
         type = gsub("diff_ext_otherfrag","Extremely vs. Other Fragile Contexts",type))

ggplot(data = dat_global_foranalysis_combined, 
       mapping = aes(x = reorder(year,-year), 
                     y = ifelse(test = type == "Extremely vs. non-Fragile Contexts", yes = -value, no = value), 
                     fill = type)) +
  geom_col() +
...
dat_global_foranalysis_combined_export <- dat_global_foranalysis_combined %>% 
  pivot_wider(names_from = "type",values_from = "value")
write.xlsx(dat_global_foranalysis_combined_export,"./graphs/growing_gap_data.xlsx")
...
dat_subregion <- group_overtime_scores_raw %>% 
  ...
  map(function(x) x %>% 
        mutate(dimension = gsub(".PC1","",dimension)) %>% 
        filter(group %in% c("African Great Lakes",
                            "Lake Chad Basin",
                            "Horn of Africa",
                            "Sahel")))
...
dat_subregion_forviz <- dat_subregion %>% 
  map(function(x) x %>%
        ...
        pivot_longer(names_to = "year",
                     values_to = "value",-c(group,dimension)) %>% 
        mutate(year = as.numeric(year)))
names(dat_subregion_forviz) <- c("Economic","Environmental",
                                 "Political","Security","Societal")

dat_subregion_viz <- imap(dat_subregion_forviz,
                          ~ggplot(data = .x,
                                  mapping = aes(x = year,
                                                y = value,
                                                group = group)) +
...
ggsave("./graphs/dimensional_fragility/subregion_graphs.pdf", marrangeGrob(grobs = dat_subregion_viz,
                                                                           nrow = 1,ncol = 1))
write.xlsx(dat_subregion_forviz,"./graphs/dimensional_fragility/subregion_dataset.xlsx")

# Gender Inequality ------------------------------------------------------
sfr_gender_ineq <- sfr.time.series %>% 
  filter(variablename == "Gender inequality (R)",
         year == 2018,
         iso3c %in% unique(ODA_recipients$iso3c)) %>% 
  select(iso3c,gender_ineq = imputed) %>%
  left_join(raw.data %>% 
              filter(year == 2019) %>% 
              select(ends_with(".PC1"),iso3c),"iso3c") %>%
  mutate_at(vars(-c(iso3c,gender_ineq)),function(x)-x) %>% 
  pivot_longer(names_to = "variable",
               values_to = "value",-iso3c) %>% 
  mutate(variable = gsub("gender_ineq","Gender Inequality",variable),
         variable = gsub(".PC1","",variable)) %>% 
  pivot_wider(names_from = "variable",
              values_from = "value") %>% 
  select(-Aggregate)

sfr_gender_ineq_export <- sfr_gender_ineq %>% 
  select(-iso3c,Security,Societal)
write.xlsx(sfr_gender_ineq_export,"./graphs/Gender inequality StatLinks.xlsx")
library(ggpmisc)
ggplot(data = sfr_gender_ineq,
       aes(x = `Gender Inequality`)) +
  geom_smooth(aes(y = Economic,
                  colour = "Economic"),
              method = "lm",
              formula = y~x,
              se = F) +
  geom_smooth(aes(y = Environmental,
                  colour = "Environmental"),
              method = "lm",
              formula = y~x,
              se = F) +
  geom_smooth(aes(y = Security,
                  colour = "Security"),
              method = "lm",
              formula = y~x,
              se = F) +
...
peace_gender <- fread("./data/snapshot data/pax_wgg_all_agreements_data.csv") %>% 
  clean_names() 

peace_gender_total <- peace_gender %>%
  filter(loc1iso %in% unique(aggr.pca_adj$iso3c)|loc2iso %in% unique(aggr.pca_adj$iso3c)) %>% 
...
peace_gender_sub <- peace_gender_total %>% 
  filter(type %in% c("wgg_par","wgg_grp","wgg_eq","wgg_int_law","wgg_new_inst","wgg_vio","wgg_tra_jus","wgg_inst_ref","wgg_dev","wgg_impl","wgg_oth")) %>% 
  mutate(type = gsub("wgg_dev","Development",type),
